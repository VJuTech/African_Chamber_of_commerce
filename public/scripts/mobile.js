(() => {
  ...
  const createInstallationId = browserCrypto && typeof browserCrypto.randomUUID === "function"
    ? () => browserCrypto.randomUUID()
    : () => `acc-${Date.now()}-${Math.random().toString(36).slice(2)}`;
  const installationId = localStorage.getItem("acc-mobile-installation") || createInstallationId();
