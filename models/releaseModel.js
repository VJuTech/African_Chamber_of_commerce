...
async function getDashboard() {
  const [versions, plans, notes, compatibility, monitoring, flags, audit] = await Promise.all([
    ...
    pool.query("SELECT a.id, a.event_type AS \"eventType\", a.outcome, a.details, a.created_at AS \"createdAt\", v.version FROM release_audit_logs a LEFT JOIN release_versions v ON v.id=a.version_id ORDER BY a.created_at DESC LIMIT 40"),
  ]);
